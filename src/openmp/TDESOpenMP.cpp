...
TDESOpenMP::~TDESOpenMP() {}

void TDESOpenMP::prepareKeys() {
	for (int k = 0; k < 3; ++k) {
...
uint64_t TDESOpenMP::processBlock(uint64_t block, int key, bool decode) {
	// Initial permutation
...
std::string TDESOpenMP::encode(std::string message) {
	this->prepareKeys();
...
std::string TDESOpenMP::decode(std::string message) {
	this->prepareKeys();
