...
int main(int argc, char const *argv[]) {
	...
	// Prepare encoder
	TDESOpenMP openMPEncoder(key);

	// Start time measurement
	auto start = std::chrono::high_resolution_clock::now();

	// Encode and decode
	std::string encodedMessage = openMPEncoder.encode(originalMessage);
	std::string decodedMessage = openMPEncoder.decode(encodedMessage);
